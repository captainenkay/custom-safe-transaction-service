...
class TracingManager(EthereumClientManager):
    ...
    def transform_debug_trace_to_trace_block(self, debug_trace):
        trace_block = []
        for tx in debug_trace["result"]:
            trace = {
                "action": tx.get("action", {}),
                "blockHash": tx.get("blockHash"),
                "blockNumber": tx.get("blockNumber"),
                "result": tx.get("result", {}),
                "subtraces": tx.get("subtraces", 0),
                "traceAddress": tx.get("traceAddress", []),
                "transactionHash": tx.get("transactionHash"),
                "transactionPosition": tx.get("transactionIndex", 0),
                "type": tx.get("type", "call"),
            }
            trace_block.append(trace)
        return trace_block
    ...
    def trace_blocks(
        ...
        # results = self.transform_debug_trace_to_trace_block(results)

        return [trace_list_result_formatter(block_traces) for block_traces in results]  # type: ignore[arg-type]

    def convert_debug_to_trace_transaction(self, trace):
        tx_trace = []

        def parse_call(call, parent=None):
            trace_obj = {
                "action": {
                ...
                "traceAddress": []
                if parent is None
                else parent["traceAddress"] + [parent["subtraces"]],
                "type": call.get("type", "call").lower(),
            }
            tx_trace.append(trace_obj)

            for sub_call in call.get("calls", []):
                parse_call(sub_call, trace_obj)

        parse_call(trace["result"])
        return tx_trace
    ...
    def trace_transaction(self, tx_hash: EthereumHash) -> List[FilterTrace]:
        ...
        """
        tx_receipt = self.slow_w3.eth.get_transaction_receipt(tx_hash)

        block_trace = self.slow_w3.manager.request_blocking(
            "debug_traceBlockByNumber",
            [hex(tx_receipt["blockNumber"]), {"tracer": "callTracer"}],
        )

        for trace in block_trace:
            if trace.get("txHash") == tx_hash:
                return self.convert_debug_to_trace_transaction(trace)
